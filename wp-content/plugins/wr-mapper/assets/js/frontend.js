...
jQuery( function( $ ) {
	...
	$( document ).ready( function() {
		var renderPin = function() {
			var map = $( '.wr-mapper' ), pin = $( '.wrm-pin' ), _img_width = map.data( 'width' ), _img_height = map.data( 'height' );
		...
		var initPopup = function() {
			var pin = $( '.wrm-pin .action-pin' );

			pin.on( 'click', function() {

				var _this  = $( this ),
					popup  = _this.siblings( '.wrm-popup' );

				if( ! popup.length ) {
					return;
				}

				var parent = _this.closest( '.wrm-pin' );
				...
				$( '.wr-mapper .wrm-pin .wrm-popup-header h2' ).css( 'max-width', popup_width - 10 );

				// Set height content for image type
				if( popup.hasClass( 'wrm-image' ) ) {
					var popup_header_height = popup.find( '.wrm-popup-header' ).outerHeight( true );
					popup.find( '.wrm-popup-main' ).css( 'height', ( popup_height - popup_header_height ) );
				}

				// Add Actived class
				setTimeout( function(){
					// Remove all pin actived class
					$( '.wr-mapper .wrm-pin.actived' ).removeClass( 'actived' );
			...
			$( '.wrm-pin .close-modal' ).on( 'click', function() {
				var parent = $(this).closest( '.wrm-pin' ),
					popup  = parent.find( '.wrm-popup' );
			...
			pin.hover( function() {
				...
				flag = false;
				_this.closest( '.wr-mapper' ).children( 'img' ).removeAttr( 'style' );
