...
class Opal_Themecontrol_Helper {
	...
	public static function get_author() {
		return '<span class="entry-author" ' . Opal_Themecontrol_Helper::schema_metadata( array( 'context' => 'author', 'echo' => false ) ) . '><i class="fa fa-user"></i><a href="' . get_author_posts_url( get_the_author_meta( 'ID' ) ) . '">' . get_the_author() . '</a></span>';
	}
	...
	public static function read_more() {
		return '<a class="more-link dt" href="' . get_permalink() . '">' . esc_html__( 'Read more', 'opal-themecontrol' ) . '<span class="dib ts-03">&rarr;</span></a>';
	}
	...
	public static function get_comment_count() {
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link( esc_html__( '0 Comment', 'opal-themecontrol' ), esc_html__( '1 Comment', 'opal-themecontrol' ), esc_html__( '% Comments', 'opal-themecontrol' ) );
		...
		switch ( $comment->comment_type ) {
			...
			case 'trackback' :
				...
				<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
					<p>
						<?php
						_e( 'Pingback:', 'opal-themecontrol' );
						comment_author_link();
						edit_comment_link( esc_html__( 'Edit', 'opal-themecontrol' ), '<span class="edit-link">', '</span>' );
						?>
					</p>
				<?php
			break;
				...
				<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
					...
					?>>
						...
						<div class="comment-content-wrap overlay_bg">
							...
							<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'opal-themecontrol' ); ?></p>
							<?php endif; ?>
							<header class="comment-meta">
								...
								<a href="<?php echo '' . get_comment_link( $comment->comment_ID ); ?>">
									<time <?php self::schema_metadata( array( 'context' => 'comment_time' ) ); ?>>
										<span> - </span>
										<?php sprintf( __( '%1$s', 'opal-themecontrol' ), comment_date() ); ?>
									</time>
								</a>
							</header>
							...
							<div class="action-link">
								<?php
								edit_comment_link( esc_html__( 'Edit', 'opal-themecontrol' ) );

								comment_reply_link(
									array_merge(
										$args,
										array(
											'reply_text' => esc_html__( 'Reply', 'opal-themecontrol' ),
											'depth'      => $depth,
										)
									)
								);
	...
	public static function schema_metadata( $args ) {
		...
		$args = apply_filters( 'opal_theme_schema_metadata_args', wp_parse_args( $args, $default_args ) );
		...
		$attributes = apply_filters( 'opal_theme_schema_metadata_attributes', $attributes, $args );
		...
		$markup = apply_filters( 'opal_theme_schema_metadata_output', $markup, $args );
	...
	public static function pagination( $nav_query = false ) {
		...
		$style = $opal_themecontrol_options['pagination_style'];

		// Right to left
		$rtl = $opal_themecontrol_options['rtl'];
		if ( $rtl ) {
	...
	public static function page_title() {
		...
		if ( is_home() && get_option( 'page_for_posts' ) ) {
			...
			esc_html_e( 'Home', 'opal-themecontrol' );
		} elseif ( function_exists( 'is_shop' ) && is_shop() && $opal_themecontrol_options['wc_archive_page_title'] ) {
			echo esc_html( $opal_themecontrol_options['wc_archive_page_title_content'], 'opal-themecontrol' );
			...
			esc_html_e( 'Search Results', 'opal-themecontrol' );
		} else {
			the_title();
		}
	}
	...
	public static function get_sidebars() {
		...
		// Get custom sidebars.
		$custom_sidebars = get_option( 'opal_theme_sidebars' );

		// Prepare output.
		$output = array();

		if ( is_customize_preview() ) {
			$output[] = esc_html__( '-- Select Sidebar --', 'opal-themecontrol' );
		}
	...
	public static function format_column_date( $post, $now = FALSE ) {
		...
		if ( '0000-00-00 00:00:00' == $post->post_date ) {
			$t_time = $h_time = __( 'Unpublished', 'opal-themecontrol' );
			$time_diff = 0;
		} else {
			...
			if ( ( $time_diff > 0 && $time_diff < DAY_IN_SECONDS ) || $now ) {
				$h_time = sprintf( __( '%s ago', 'opal-themecontrol' ), human_time_diff( $time ) );
			} else {
	...
	/**
	 * Add font to list url google fonts in filter opal_font_url.
	 *
	 * @param $list_fonts array()
	 *
	 * @return  array
