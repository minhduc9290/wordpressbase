...
class Opal_Themecontrol_Export {
	...
	public function __construct() {
		...
		add_action( 'admin_menu', array( &$this, 'opalthemer_themer_admin_export' ) );
	...
	public function export_data_all(){
		$actions = array(
			'export_page_options',
			'export_opalthemer_themer_menus',
			'export_sidebars',
		...
		do_action( 'opalthemer_themer_export_all_data' );
	...
	public function export_wordpress_content_file(){
		...
		if ( !current_user_can('export') )
			wp_die(__('You do not have sufficient permissions to export the content of this site.', 'opal-themecontrol'));
	...
	public function export_wordpress_content(){
		...
		/** Load WordPress export API */
		require_once( ABSPATH . 'wp-admin/includes/export.php' );
		$title = __('Export', 'opal-themecontrol');
	...
	public function export_data_modes() {
		$opalthemer_themer_options = get_option( "opalthemer_theme_options" );
		$output            = base64_encode( serialize( $opalthemer_themer_options ) );
	...
	public function export_widgets() {
		...
		$all_opalthemer_themer_widgets = array();

		foreach ( $wp_registered_widgets as $opalthemer_themer_widget_id => $widget_params ) {
			$all_opalthemer_themer_widgets[] = $widget_params['callback'][0]->id_base;
		}
		$widget_datas = array();
		foreach ( $all_opalthemer_themer_widgets as $opalthemer_themer_widget_id ) {
			$opalthemer_themer_widget_data = get_option( 'widget_' . $opalthemer_themer_widget_id );
			if ( ! empty( $opalthemer_themer_widget_data ) ) {
				$widget_datas[ $opalthemer_themer_widget_id ] = $opalthemer_themer_widget_data;
			}
		}
		unset( $all_opalthemer_themer_widgets );
	...
	public function export_sidebars() {
		$opalthemer_themer_sidebars = get_option( "sidebars_widgets" );
		$opalthemer_themer_sidebars = $this->exclude_sidebar_keys( $opalthemer_themer_sidebars );

		return $opalthemer_themer_sidebars;
	}
	...
	public function export_opalthemer_themer_menus() {
		global $wpdb;
	...
	public function export_page_options() {  
		$opalthemer_themer_static_page    = get_option( "page_on_front" );
		$opalthemer_themer_post_page      = get_option( "page_for_posts" );
		$opalthemer_themer_show_on_front  = get_option( "show_on_front" );
		$opalthemer_themer_settings_pages = array(
			'show_on_front'  => $opalthemer_themer_show_on_front,
			'page_on_front'  => $opalthemer_themer_static_page,
			'page_for_posts' => $opalthemer_themer_post_page
		);
		$output                   = base64_encode( serialize( $opalthemer_themer_settings_pages ) );
	...
	public function opalthemer_themer_admin_export() {
		...
		add_submenu_page( 'themes.php', __( 'WpOpal Export', 'opal-themecontrol' ), esc_html__( 'WpOpal Export', 'opal-themecontrol' ), 'manage_options', 'opalthemer_themer_options_export_page', array(
			&$this,
			'opalthemer_themer_generate_export_page'
		) );
	}
	...
	public function opalthemer_themer_generate_export_page() {
		...
		<div class="wrapper">
			<div class="content">
				<table class="form-table">
					<tbody>
					<tr>
						<td scope="row" width="150"><h2><?php esc_html_e( 'Export Wordpress Content', 'opal-themecontrol' ); ?></h2></td>
					</tr>
					...
					<tr valign="middle">
						...
						<td>
							...
							<form method="post" action="">
								<input type="hidden" name="export_data_mode" value="opalthemer_themer_menus"/>
					...
					<tr>
						<td scope="row" width="150"><h2><?php esc_html_e( 'Export Revolution Slider', 'opal-themecontrol' ); ?></h2></td>
					</tr>
...
$my_Opal_Themecontrol_Export = new Opal_Themecontrol_Export();