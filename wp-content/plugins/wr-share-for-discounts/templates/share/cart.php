...
?>
<tr>
	<td colspan="6" class="actions <?php if ( isset( $settings['cart_discount_message'] ) && ! empty( $settings['cart_discount_message'] ) ) echo 'has-message'; ?>">
		<div class="wr-product-share" data-type="<?php echo esc_attr( $type ); ?>">
			<?php if ( isset( $discount['cart_discount'] ) && ! empty( $discount['cart_discount'] ) ) : ?>
			...
			<p>
				...
				<br>
				<strong><?php echo esc_html( $discount['cart_discount'] ); ?></strong>
			</p>
			<?php else : ?>
			<h3><?php _e( 'Share for Discounts', 'wr-share-for-discounts' ); ?></h3>
			<?php
			if ( isset( $settings['cart_discount_message'] ) && ! empty( $settings['cart_discount_message'] ) ) {
				echo '<div class="wr-share-message">' . wp_kses_post( str_replace( '\\', '', $settings['cart_discount_message'] ) ) . '</div>';
			}

			echo '<div class="share-boxs">';

			if ( $settings['enable_facebook_sharing'] ) :
			?>
			<div id="fb-root"></div>
			<div class="facebook-sharing">
				<?php if ( 'share' != $settings['facebook_button_type'] ) : ?>
				<div class="fb-like"
					data-href="<?php echo '' . $settings['cart_discount_link']; ?>"
					data-layout="button"
					data-action="like"
				...
				?>
				<a class="wr-share-to-fb-button" href="javascript:void(0)" data-href="<?php echo '' . $settings['cart_discount_link']; ?>">
			...
			<div class="twitter-sharing">
				<a href="https://twitter.com/share" class="twitter-share-button"
					data-count="none"
					data-url="<?php echo '' . $settings['cart_discount_link']; ?>"
